...
Formally verified Application Binary Interface (ABI) for cryptographic
hashing operations.
...
[arabic]
. *Size Safety*: Hash output size encoded in types
+
[source,idris]
----
blake3Hash : Vect n Byte -> IO (HashValue 32)  -- Always 32 bytes
----
. *No Padding*: Byte arrays are tightly packed
+
[source,idris]
----
hashValueNoPadding : NoPadding (HashValue n)
----
. *Platform Independent*: Layout doesn’t depend on architecture
+
[source,idris]
----
hashValueLayoutIndependent : PlatformIndependent (hashValueLayout n)
----
. *C Compatible*: Matches `+uint8_t data[n]+` layout
+
[source,idris]
----
...
[width="100%",cols="31%,21%,24%,24%",options="header",]
|===
|Function |Input |Output |Safety
|`+blake3_hash+` |`+const uint8_t*, size_t+` |`+uint8_t[32]+` |Buffer
overflow impossible (Zig slice)

|`+sha256_hash+` |`+const uint8_t*, size_t+` |`+uint8_t[32]+` |Buffer
overflow impossible (Zig slice)

|`+sha3_256_hash+` |`+const uint8_t*, size_t+` |`+uint8_t[32]+` |Buffer
overflow impossible (Zig slice)
|===

=== Test Vectors

Empty string hashes (for ABI verification): - BLAKE3:
